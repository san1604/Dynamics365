...
public class SaveandSyncQuote : IPlugin
{
    ...
    public void Execute(IServiceProvider serviceProvider)
    {
        ...
        try
        {
            // ── ENV VARIABLES ─────────────────────────────────────────
            string apiBaseUrl =
                GetEnvironmentVariable(orgService, "sb1_apibaseurl");

            string apiKey =
                GetEnvironmentVariable(orgService, "sb1_apikey");

            // ── TOKEN FROM ENTITY ─────────────────────────────────────
            string accessToken = GetAccessTokenFromEntity(orgService);

            if (string.IsNullOrWhiteSpace(accessToken))
                throw new Exception("Access token not found in sb1_b1apitoken.");
            ...
            dynamic payload =
                JsonConvert.DeserializeObject(quotePayloadJson);

            // ── DUPLICATE CHECK ───────────────────────────────────────
            int formType = SafeInt(payload?.form_type);

            string workflowLabel =
                SafeString(payload?.workflow_category_label);
                ...
                if (!string.IsNullOrWhiteSpace(quoteName))
                {
                    ...
                    if (existingQuotes.Entities.Count > 0)
                    {
                        SetResponse(
                            context,
                            false,
                            $"Quote with name '{quoteName}' already exists.");
            ...
            // ── ACCOUNT ───────────────────────────────────────────────
            string accountGuid =
                SafeString(payload?.account_id);

            string accId = "0";

            string accName =
                SafeString(payload?.account_name);
            ...
            string jsonBody =
                JsonConvert.SerializeObject(b1Payload);

            tracing.Trace("Prepared JSON: " + jsonBody);

            // ── API CALL ──────────────────────────────────────────────
            string apiResponse = CallB1Api(
                jsonBody,
                accessToken,
                tracing,
                apiBaseUrl,
            ...
            newQuote["sb1_quoteid"] =
                folderVersionId.ToString();

            newQuote["sb1_quotefoldername"] =
                quoteIdFromApi;
            ...
            string folderMode =
                SafeString(payload?.folder_mode);

            if (!string.IsNullOrWhiteSpace(folderMode))
                newQuote["sb1_foldermode"] = folderMode;

            if (quoteFolderId > 0)
                newQuote["sb1_folderid"] =
                    quoteFolderId.ToString();

            // Account
            if (Guid.TryParse(accountGuid, out Guid accRef))
            {
                newQuote["customerid"] =
                    new EntityReference("account", accRef);
            }

            // Owner
            if (Guid.TryParse(
                SafeString(payload?.owner_id),
                out Guid ownerGuid))
            {
                newQuote["ownerid"] =
                    new EntityReference("systemuser", ownerGuid);
            }

            // Opportunity
            if (Guid.TryParse(
                SafeString(payload?.opportunity_id),
                out Guid oppGuid))
            {
                newQuote["opportunityid"] =
                    new EntityReference("opportunity", oppGuid);
            }

            // Broker
            if (Guid.TryParse(
                SafeString(payload?.broker_guid),
                out Guid brokerGuid))
            {
                newQuote["sb1_broker"] =
                    new EntityReference("contact", brokerGuid);
            }

            newQuote["sb1_brokerid"] =
                SafeString(payload?.broker_code);

            // Fields
            newQuote["sb1_lineofbusiness"] =
                SafeString(payload?.line_of_business);

            newQuote["sb1_marketsegment"] =
                SafeString(payload?.market_segment);

            newQuote["sb1_fundingarrangement"] =
                SafeString(payload?.funding_arrangement);

            newQuote["sb1_state"] =
                SafeString(payload?.state);

            newQuote["sb1_region"] =
                SafeString(payload?.region);

            // Effective Date
            if (DateTime.TryParse(
                SafeString(payload?.effective_from),
                out DateTime effFrom))
            {
            ...
            // Workflow
            int workflowVal =
                SafeInt(payload?.workflow_category_value);

            if (workflowVal >= 0)
            {
                newQuote["sb1_quotetype"] =
                    new OptionSetValue(workflowVal);
            }

            // Revision
            string revision =
                SafeString(payload?.revision_id);
            ...
            // Numeric Fields
            int subscribers =
                SafeInt(payload?.number_of_subscribers);

            if (subscribers > 0)
                newQuote["sb1_numberofsubscribers"] =
                    subscribers;

            int members =
                SafeInt(payload?.number_of_members);

            if (members > 0)
                newQuote["sb1_numberofmembers"] =
                    members;

            int minAge =
                SafeInt(payload?.min_age);
            ...
            int avgAge =
                SafeInt(payload?.average_age);

            if (avgAge > 0)
                newQuote["sb1_averageage"] =
                    avgAge;

            Guid newQuoteId =
                orgService.Create(newQuote);
    ...
    {
        string url =
            $"{apiBaseUrl}/Quote/Blank/New?api_key={apiKey}";
    ...
    // ── TOKEN FETCH ────────────────────────────────────────────────
    private string GetAccessTokenFromEntity(
        IOrganizationService service)
    {
        var query = new QueryExpression("sb1_b1apitoken")
        {
        ...
        return result.Entities[0]
            .GetAttributeValue<string>("sb1_accesstoken");
    }
