...
function toggleMultiSelect(id) {
    ...
    document.querySelectorAll('.multi-select-wrap.open').forEach(w => w.classList.remove('open'));
...
function setMultiSelectValues(id, commaSeparated) {
    const wrap = document.getElementById(id + '-wrap');
    if (!wrap) return;
    const values = commaSeparated ? commaSeparated.split(',').map(v => v.trim()).filter(Boolean) : [];
...
async function saveForm(form) {
    ...
    const data = {
        ...
        sb1_internalreviewfornew: getValue("wf-new-internal"),
        sb1_internalreviewforrenew: getValue("wf-renew-internal"),
        sb1_brokervalidationfornew: getValue("wf-new-broker"),
        sb1_brokervalidationforrenew: getValue("wf-renew-broker")
    };
...
async function loadWorkflowConfig() {
    ...
    try {
        ...
        setMultiSelectValues("wf-new-internal", record.sb1_internalreviewfornew);
        setMultiSelectValues("wf-new-broker", record.sb1_brokervalidationfornew);
        ...
        setMultiSelectValues("wf-renew-internal", record.sb1_internalreviewforrenew);
        setMultiSelectValues("wf-renew-broker", record.sb1_brokervalidationforrenew);
